...
public class ViewDeviations {
	...
	protected void createContents(Display display) {
        ...
        if(score.isCollected()){
        	HashMap<String,Object> one=score.getDeviations(matchName, matchType);
        	DecimalFormat format=new DecimalFormat("#0.0000");
        	if(one!=null){
        		item=new TableItem(table,SWT.NONE);
            	item.setText(new String[]{format.format(one.get("arterror1")),format.format(one.get("arterror2")),format.format(one.get("arterror3")),format.format(one.get("arterror4"))
            			,format.format(one.get("execerror1")),format.format(one.get("execerror2")),format.format(one.get("execerror3")),format.format(one.get("execerror4"))
            			,format.format(one.get("imperror1")),format.format(one.get("imperror2"))});
        	}
        	if(score.isCollected()){
        		score.close();
        	}
