	...
	protected void createContents(Display display) {
		...
		if(finalTeamExport.isCollected()){
            ...
            for(int i=0;i<teams.size();i++){
            	...
            	item.setText(new String[]{teams.get(i).get("category").toString(),
		...
		button.addSelectionListener(new SelectionAdapter(){
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				for(int i=0;i<teams.size();i++){
					...
					}else if(!teams.get(i).get("leftCount").toString().trim().equals("0")&&!teams.get(i).get("exportCount").toString().trim().equals("0")){  
						leftCountmention+=teams.get(i).get("category").toString()+"\n";    //没有比赛完的队伍
					}else{
						int count;
						try{
							count=Integer.valueOf(text);
							if(count<0){
								negativemention+=teams.get(i).get("category").toString()+"\n";  //填写要导出队伍人数为负值的项目
							}else if(count>Integer.valueOf(teams.get(i).get("hasMatchCount").toString())){
								biggermention+=teams.get(i).get("category").toString()+"\n";
							}
						}catch(Exception e){
							negativemention+=teams.get(i).get("category").toString()+"\n";
						}						
                ...
                if(mark){
                	if(finalTeamExport.isCollected()){
						...
						for(int i=0;i<teams.size();i++){
							if(counts.get(i).getText()!=null&&!counts.get(i).getText().equals("0")){
								one=teams.get(i);
								one.put("selectCount", counts.get(i).getText().trim());
								data.add(one);
							}
						}
						...
						if(fileName != null && !fileName.equals("")){
							...
							if(manager.ExportExcelFinal(finalTeamExport.getFinalTeamAll(data, matchName), fileName, matchName, "", true)){
	 ...
	 private void createTableText(Table table,List<HashMap<String,Object>> items) {
