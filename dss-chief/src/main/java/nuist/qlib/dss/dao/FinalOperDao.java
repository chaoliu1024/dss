/*
 * 文件名：FinalOperDao.java
 * 版权：Copyright 2014 Artisan WangFang
 * 描述：将web_json中的sort_flat以及决赛标志位更改，变成决赛为排序队伍
 */
...
public class FinalOperDao {
	// 连接数据库需要的变量
		...
		public List<HashMap<String,Object>> getAllCategory(String matchName){
			...
			List<HashMap<String,Object>> result=connSql.selectQuery(sql, new Object[]{matchName,matchName});
			...
			for(int i=0;i<result.size();i++){
				...
				subResult=connSql.selectQuery(sql, new Object[]{matchName,category});
				if(subResult.size()!=0){
					result.get(i).put("leftCount", subResult.get(0).get("leftCount"));
				}
				...
				subResult=connSql.selectQuery(sql, new Object[]{matchName,category});
				if(subResult.size()!=0){
					result.get(i).put("hasMatchCount", subResult.get(0).get("hasMatchCount"));
				}
				...
				subResult=connSql.selectQuery(sql, new Object[]{matchName,category});
				if(subResult.size()!=0){
					result.get(i).put("abstainCount", subResult.get(0).get("abstainCount"));
				}
			}
			return result;			
		}
		 ...
		 * @return
		 */
		public List<HashMap<String,Object>> getTeamByCategory(int count,String matchName,String category){
			...
			List<HashMap<String,Object>> result=connSql.selectQuery(sql, new Object[]{count,category,matchName});
			 int i = 1;
				HashMap one=null;
				String score="";
				if(result.size()!=0){
		 ...
		 * @return
		 */
		public List<HashMap<String,Object>> getTeamByCategory(String matchName,String category){
			String sql="select b.match_units as teamName,b.match_category as category,b.member_name as memberName from score as a,match_order as b where a.team_id=b.id and b.match_category=? and b.match_name=? and b.final_preliminary=0 " +
	    			"order by a.total desc";
			List<HashMap<String,Object>> result=connSql.selectQuery(sql, new Object[]{category,matchName});
			return result;
		}
}
